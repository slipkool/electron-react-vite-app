...
export const deleteClient = createAsyncThunk('clients/delete', async (id: number) => {
  const result = await ClientService.deleteClient(id)
  return result
})

export const addClient = createAsyncThunk('clients/create', async (client: CreateClientDto) => {
  const result = await ClientService.addClient(client)
  return clientDtoToClientModel(result)
})

export const updateClient = createAsyncThunk('clients/update', async (client: UpdateClientDto) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteClient.fulfilled, (state, action) => {
        ...
        if (id) {
          state.clients = state.clients.filter((ele) => ele.id !== parseInt(id))
        }
      })
